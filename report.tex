...
\newcommand\theResearchTopic{\textit{{\large\bfseries AI-Driven Personal Budgeting and Financial Literacy Enhancement for University Students in Uganda}}}

\newcommand\theStudentOneName{\textit{Mukama Joseph}}
\newcommand\theStudentOneRegNo{\textit{S23B23/036}}
\newcommand\theStudentTwoName{\textit{Oscar Odongkara}}
\newcommand\theStudentTwoRegNo{\textit{S23B23/085}}
\newcommand\theStudentThreeName{\textit{Namaganda Wakabi Precious}}
\newcommand\theStudentThreeRegNo{\textit{S23B23/092}}
\newcommand\theSpecializationTrack{\textit{Specialization: Software Engineering}}

\newcommand\thePrimaryAdvisor{\textit{[SUPERVISOR NAME]}} %Please seek consent
\newcommand\theOtherAdvisor{} 

\newcommand\theProgrameAward{Bachelor of Science in Computer Science}
\newcommand\theTheDocumentType{Report} %[Enter either proposal or Report or dissertation]
...
\newcommand\theVersionDate{\textit{March 2026}}

%%%%%=====================. %%%%

\newcommand\makLogo{\includegraphics[scale=.25]{Logo.png}} % Changed path to use local Logo.png
...
\begin{document}
    ...
    \begin{titlepage}
        ...
        {\small\bfseries \theDepartment}\\[0.05cm]
        {\small \theFaculty}\\[1.2cm]
        ...
        \Large{
            ...
            \begin{tabular}{ll}
            \textbf{\theStudentOneName} & \theStudentOneRegNo \\[4pt]
            \textbf{\theStudentTwoName} & \theStudentTwoRegNo \\[4pt]
            \textbf{\theStudentThreeName} & \theStudentThreeRegNo \\
            \end{tabular}\\[1.5cm]
            
            \vspace{1cm}
            
            A \theTheDocumentType~submitted to the \theFaculty~in partial fulfilment of the requirements for the award of the \textbf{\theProgrameAward}~of Uganda Christian University \\
            
            \vspace{1cm}
            
            \theSpecializationTrack\\
              \vspace{1cm}
            \thePrimaryAdvisor \\
            \theOtherAdvisor \\
...
\addcontentsline{toc}{chapter}{Declaration}
This project utilised AI responsibly for code formatting, generation of generic templates, and proofreading, while preserving the intellectual originality of the core logic and research output.
...
Random Forest anomaly detection architecture optimised
...
Dedicated to our families for their endless support.
...
\begin{enumerate}
    \item To design and implement an automatic expense ingestion system capable of parsing unstructured SMS notifications from MTN MoMo and Airtel Money using the DocTR and Tesseract OCR models for receipt parsing and a lightweight Gaussian Naive Bayes categorisation model.
    \item To develop and deploy deep learning forecasting models based on Random Forest classification models, optimised via Grid Search for accurate prediction of future spending patterns and savings trajectories from limited historical data.
...
\begin{description}[leftmargin=3cm, style=nextline]
    ...
    \item[Joblib serialized models] A lightweight version of Scikit-Learn designed for on-device machine learning inference on mobile and embedded systems.
...
tasks. Random Forest and Isolation Forest models
...
records \citep{Kumar2024}. Ensemble architectures combining tree-based
...
\begin{itemize}
    ...
    \item \textbf{Application Telemetry:} Continuous behavioural data collected via System Analytics during the pilot, including session length, feature usage frequency, lesson completion rates, and gamification engagement metrics.
...
\begin{description}
    \item[Frontend:] React framework with TypeScript, implementing the Material 3 design system. All core functionality operates without internet connectivity; synchronisation occurs opportunistically when connectivity is available.
    \item[Local Data Layer:] Supabase PostgreSQL database stores all transaction records, budget configurations, savings goals, gamification state, and user profiles entirely on-device.
    \item[AI Subsystem:] Includes DocTR and Tesseract OCR for receipt text extraction, paired with Random Forest anomaly detection pipelines. Models trained offline in Python 3.11 (Scikit-Learn 2.16, Keras, scikit-learn) and exported to Joblib serialized models format with int8 quantisation, targeting a file size below 5 MB for on-device inference.
    \item[Backend Services:] Supabase Authentication for secure user management, Supabase PostgreSQL for minimal synchronised data (leaderboard scores, anonymised community challenge participation), and Cloud Functions for server-side leaderboard computation.
\end{description}
...
\begin{enumerate}
    \item The Receipt OCR Subsystem extracts transaction data from uploaded receipt images.
    ...
    \item A Random Forest classifier (transaction\_rf\_pipeline.pkl) assigns each transaction to one of 18 locally defined categories (food, transport, utilities, airtime/data, entertainment, educational expenses, church/tithe, clan contributions, funeral support, wedding/social, savings, income -- allowance, income -- work, income -- transfer, health, clothing, accommodation, and miscellaneous).
...
The forecasting sub-system employs a hybrid Random Forest architecture:

\begin{itemize}
    \item \textbf{Architecture:} 2--3 stacked Random Forest estimators (64--128 units each) followed by dense layers for simultaneous processing of sequential transaction features and static user features (student year, accommodation type, income regularity score).
    \item \textbf{Optimisation:} Particle Swarm Optimization (PSO) performs automated hyperparameter search over learning rate, number of Random Forest estimators, number of units per layer, dropout rate, and batch size.
    ...
    \item \textbf{Deployment:} Exported to Joblib serialized models (int8 quantised) for on-device inference with no internet requirement.
\end{itemize}
...
\begin{itemize}
    ...
    \item Anonymous, aggregated, opt-in data may be shared for community challenge leaderboard computation via Supabase, with explicit user consent.
\end{itemize}
...
\begin{description}
    ...
    \item[Phase 3 -- AI \& Gamification Integration:] Random Forest model training, Joblib serialized models export and integration, gamification engine implementation, and financial literacy lesson content population.
...
Weeks 4--6\newline(18 Dec -- 14 Jan) & Development of authentication, Receipt OCR scanner, manual entry interface; initial Random Forest model training on simulated data & Functional MVP v0.1 with automatic expense ingestion \\[4pt]
...
Google Play Console one-time fee and Supabase Pro plan & 250,000 \\
...
At the device level, the React frontend manages all user interactions, renders the
...
state. The AI subsystem runs entirely on-device using Joblib serialized models models. At the
cloud tier, Supabase services handle authentication, leaderboard computation, and
...
classified by a Random Forest model trained on TF-IDF features extracted from
...
The forecasting engine employs a hybrid architecture comprising stacked Random Forest estimators
for sequential transaction modelling followed by dense layers for static feature
integration:

\begin{lstlisting}[language=Python, caption={Random Forest Forecasting Model Architecture}]
...
def build_uniguard_forecasting_model(
        ...
        dropout_rate=0.2):
    """
    Hybrid Random Forest model for student spending forecasting.
...
$\alpha \in [10^{-4}, 10^{-2}]$, Maximum depth $u \in \{32, 64, 128, 256\}$,
number of Random Forest estimators $L \in \{1, 2, 3\}$, dropout rate $d \in [0.1, 0.5]$, and
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mukama/.gemini/antigravity/brain/090a4a62-df78-4ba8-8882-18b00cbe6561/login_page_1776453699653.png}
...
\begin{enumerate}
    ...
    \item \textbf{Forecast:} Random Forest-generated spending predictions for the next 7 and 30 days, with savings trajectory chart.
...
SMS Received $\to$ Regex Parser $\to$ Random Forest Classifier $\to$
SQLite Storage $\to$ Dashboard Update $\to$ Budget Check $\to$
Anomaly Detection Update $\to$ Gamification Event $\to$ Nudge Evaluation
\vspace{1cm}}}
...
Actors: Student User, System AI, Supabase Cloud\\
...
\begin{enumerate}
    ...
    \item \textbf{SMS Corpus Construction and Parser Development:} The 5,000-message annotated corpus was assembled, regex patterns were iteratively developed and validated, and the Random Forest categorisation model was trained.
    \item \textbf{Flutter Frontend Development:} The application UI was implemented in React following the high-fidelity Figma prototypes produced in the design phase, with full offline-first architecture.
    \item \textbf{AI Model Training and Pickle/Joblib Deployment:} The Random Forest model was trained in Python/Scikit-Learn, hyperparameters were tuned via PSO, and the final model was quantised and exported to Joblib serialized models for on-device inference.
...
Table~\ref{tab:sms_results} presents the Receipt OCR scanner evaluation results on the held-out
...
\begin{table}[H]
\centering
\caption{Random Forest Category Classifier Performance (Test Set, $n = 950$)}
...
Random Forest (PSO-tuned)     & \textbf{14,320} & \textbf{21,580} & \textbf{8.9} \\
\bottomrule
\end{tabular}
\end{table}
...
rate $= 0.00087$, Random Forest estimators $= 2$, units per layer $= 128$ (layer 1) and $64$
...
Random Forest inference latency      & $<$150 ms & On Tecno Spark 8 (2 GB RAM) \\
Pickle/Joblib model size           & 3.8 MB    & int8 quantised \\
...
The Random Forest anomaly detector requires a minimum of 30 days of transaction history to
...
on-device Random Forest spending forecasting, a gamification engine grounded in
...
overall accuracy, the Random Forest forecasting model achieved a Mean Absolute Error of
...
\begin{itemize}
    ...
    \item On-device Random Forest forecasting with PSO-tuned hyperparameters achieving 8.9\% MAPE on student spending prediction.
...
\begin{description}
    ...
    \item[Mukama Joseph (S23B23/036):] Literature review, AI methodology design, SMS parsing pipeline development, deep learning model training and PSO optimisation, Joblib serialized models integration, system architecture documentation, and technical documentation.
